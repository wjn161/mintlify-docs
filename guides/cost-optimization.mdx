---
title: "Optimize LLM Costs with Inferoute Routing"
...
<Steps>
  <Step title="Audit your current model usage">
    Open the [Usage dashboard](https://app.inferoute.ai/usage) and filter by model to see where your spend is concentrated. Sort by cost descending to find the highest-impact models.
  ...
  <Step title="Set usage caps per API key">
    In the [API Keys](https://app.inferoute.ai/keys) section of the dashboard, set a monthly spend limit per key. When the cap is reached, the key returns a `429` response instead of continuing to accrue charges.
  ...
  <Step title="Monitor and iterate">
    Return to the [Usage dashboard](https://app.inferoute.ai/usage) weekly. Filter by:
